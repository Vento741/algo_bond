...
import type { BotResponse, BotCreate, BotStatus, BotMode, StrategyConfig, ExchangeAccount } from '@/types/api';
  ...
  function toggleBot(id: string, currentStatus: BotStatus) {
    ...
    api
      ...
      .catch(() => {
        ...
        setBots((prev) =>
          prev.map((b) =>
            b.id === id ? { ...b, status: (currentStatus === 'running' ? 'stopped' : 'running') as BotStatus } : b,
          ),
        );
      });
  }
        ...
        <div className="relative">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
              <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-brand-premium/20 to-brand-accent/10 border border-brand-premium/20 shadow-lg shadow-brand-premium/5 flex-shrink-0">
                <Bot className="h-5 w-5 sm:h-6 sm:w-6 text-brand-premium" />
              </div>
              <div className="min-w-0">
                <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-[Tektur]">Боты</h1>
                <p className="text-xs sm:text-sm text-gray-500 mt-0.5 truncate">
                  {bots.length > 0
                    ? `${bots.length} ${(() => {
                        const n = bots.length % 100;
                        const n10 = n % 10;
                        if (n > 10 && n < 20) return 'ботов';
                        if (n10 === 1) return 'бот';
                        if (n10 >= 2 && n10 <= 4) return 'бота';
                        return 'ботов';
            ...
            <Button
              ...
              className="w-full sm:w-auto min-h-[44px] bg-brand-premium text-brand-bg hover:bg-brand-premium/90 font-semibold shadow-lg shadow-brand-premium/10"
            >
        ...
        <Card className="border-white/[0.06] bg-gradient-to-br from-white/[0.03] to-white/[0.01] hover:border-white/[0.1] transition-colors">
          <CardContent className="p-4 sm:p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-brand-accent/10 border border-brand-accent/10 flex-shrink-0">
                <Hash className="h-4 w-4 text-brand-accent" />
              </div>
            </div>
            <p className="text-2xl sm:text-3xl font-bold font-mono text-white">{bots.length}</p>
        ...
        <Card className="border-white/[0.06] bg-gradient-to-br from-white/[0.03] to-white/[0.01] hover:border-white/[0.1] transition-colors">
          <CardContent className="p-4 sm:p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-brand-profit/10 border border-brand-profit/10 flex-shrink-0">
                <Activity className="h-4 w-4 text-brand-profit" />
              </div>
            </div>
            <p className="text-2xl sm:text-3xl font-bold font-mono text-brand-profit">{activeBots}</p>
            <p className="text-xs text-gray-500 mt-1.5">{activeBots > 0 ? 'Торговля ведется' : 'Нет активных'}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-brand-premium/10 bg-gradient-to-br from-brand-premium/[0.04] to-white/[0.01] hover:border-brand-premium/20 transition-colors">
          <CardContent className="p-4 sm:p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-brand-premium/10 border border-brand-premium/10 flex-shrink-0">
                <TrendingUp className="h-4 w-4 text-brand-premium" />
              </div>
            </div>
            <p
              className={`text-2xl sm:text-3xl font-bold font-mono break-all ${totalPnl >= 0 ? 'text-brand-profit' : 'text-brand-loss'}`}
            >
        ...
        <Card className="border-white/[0.06] bg-gradient-to-br from-white/[0.03] to-white/[0.01]">
          <CardContent className="flex flex-col items-center justify-center py-12 sm:py-20 px-4">
            <div className="relative mb-6 sm:mb-8">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-brand-premium/10 to-brand-accent/5 border border-brand-premium/10 flex items-center justify-center">
                <Bot className="h-7 w-7 sm:h-9 sm:w-9 text-brand-premium/40" />
              </div>
              <div className="absolute -top-3 -right-3 w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-brand-profit/5 border border-brand-profit/10 flex items-center justify-center">
                <TrendingUp className="h-4 w-4 text-brand-profit/30" />
              </div>
              <div className="absolute -bottom-2 -left-3 w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-brand-accent/5 border border-brand-accent/10 flex items-center justify-center">
                <Zap className="h-3.5 w-3.5 text-brand-accent/30" />
              </div>
            </div>
            <h3 className="text-base sm:text-lg font-semibold text-white font-[Tektur] tracking-tight text-center">
              Создайте первого бота
            </h3>
            <p className="text-xs sm:text-sm text-gray-500 mt-2 text-center max-w-sm leading-relaxed">
              Автоматизируйте торговлю - бот будет выполнять стратегию 24/7, открывая и закрывая позиции по вашим
              правилам
            </p>
            <Button
              onClick={() => setShowCreate(true)}
              className="mt-6 min-h-[44px] bg-brand-premium text-brand-bg hover:bg-brand-premium/90 font-semibold shadow-lg shadow-brand-premium/10"
            >
              <Plus className="mr-2 h-4 w-4" />
              Создать бота
            </Button>
              ...
              >
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <CardTitle className="text-sm text-white truncate">{config?.name ?? 'Конфигурация'}</CardTitle>
                        <span
                      ...
                      <p className="text-xs text-gray-500 mt-0.5 truncate">
                        {config ? `${config.symbol} / ${config.timeframe}` : bot.strategy_config_id.slice(0, 8)}
                      </p>
                    </div>
                  </div>
                ...
                <CardContent className="px-4 pb-2 pt-1">
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    {/* P&L */}
                    <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <TrendingUp className="h-3 w-3 text-gray-600 flex-shrink-0" />
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider">P&L</span>
                      </div>
                      <p
                        className={`text-sm sm:text-base font-bold font-mono break-all ${pnl >= 0 ? 'text-brand-profit' : 'text-brand-loss'}`}
                      >
                        {pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}
                      </p>
                    </div>
                    ...
                    <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <Percent className="h-3 w-3 text-gray-600 flex-shrink-0" />
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider">Win Rate</span>
                      </div>
                      <p className="text-sm sm:text-base font-bold font-mono text-white">{winRate.toFixed(1)}%</p>
                    ...
                    <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <BarChart3 className="h-3 w-3 text-gray-600 flex-shrink-0" />
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider">Сделки</span>
                      </div>
                      <p className="text-sm sm:text-base font-bold font-mono text-white">{bot.total_trades}</p>
                    </div>

                    {/* Drawdown */}
                    <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <ArrowDownRight className="h-3 w-3 text-gray-600 flex-shrink-0" />
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider">DD</span>
                      </div>
                      <p className="text-sm sm:text-base font-bold font-mono text-brand-loss break-all">
                ...
                <div className="px-4 pb-4 pt-2">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {isError && <span className="text-[10px] text-brand-loss/70">Требуется внимание</span>}
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {isRunning ? (
                      <Button
                        ...
                        className="flex-1 min-h-[44px] bg-brand-loss/10 text-brand-loss border border-brand-loss/20 hover:bg-brand-loss/20 hover:text-brand-loss"
                      >
                      ...
                      <Button
                        ...
                        className="flex-1 min-h-[44px] bg-brand-profit/10 text-brand-profit border border-brand-profit/20 hover:bg-brand-profit/20 hover:text-brand-profit"
                      >
                    ...
                    <Button
                      ...
                      className="min-h-[44px] min-w-[44px] text-gray-500 hover:text-white hover:bg-white/[0.06]"
                      title="Подробнее"
                    >
                    ...
                    {!isRunning && (
                      <Button
                        ...
                        className="min-h-[44px] min-w-[44px] text-gray-500 hover:text-brand-loss hover:bg-brand-loss/[0.06]"
                        title="Удалить бота"
                      >
...
function CreateBotDialog({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  const [configs, setConfigs] = useState<StrategyConfig[]>([]);
      ...
      <DialogContent>
        ...
        {loadingData ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="text-sm font-medium text-gray-300 block mb-2">Конфигурация стратегии</label>
              {configs.length === 0 ? (
                <div className="flex items-center gap-3 p-4 rounded-xl border border-white/[0.08] bg-white/[0.02]">
                  ...
                  <p className="text-sm text-gray-400">
                    ...
                    <Link to="/strategies" className="text-brand-premium hover:text-brand-premium/80 underline">
            ...
            <div>
              <label className="text-sm font-medium text-gray-300 block mb-2">Аккаунт биржи</label>
              {accounts.length === 0 ? (
                <div className="flex items-center gap-3 p-4 rounded-xl border border-white/[0.08] bg-white/[0.02]">
                  ...
                  <p className="text-sm text-gray-400">
                    Добавьте API ключи{' '}
                    <Link to="/settings" className="text-brand-premium hover:text-brand-premium/80 underline">
                      в настройках
                    </Link>
                  </p>
                </div>
            ...
            <div>
              <label className="text-sm font-medium text-gray-300 block mb-2">Режим торговли</label>
                ...
                {MODE_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        relative p-2.5 sm:p-3 rounded-xl border text-left transition-all duration-200 min-h-[72px]
                        ${
                          isActive
            ...
            {mode === 'live' && (
              <div className="p-4 rounded-xl border border-brand-loss/20 bg-brand-loss/[0.04]">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-brand-loss mb-1">Реальные средства</p>
                    <p className="text-xs text-brand-loss/70 leading-relaxed">
                      Режим Live использует реальные средства. Убедитесь в корректности стратегии перед запуском.
                      Протестируйте сначала в Demo.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button type="button" variant="ghost" onClick={onClose} className="flex-1 min-h-[44px] text-gray-400">
                Отмена
              </Button>
              <Button
                type="submit"
                disabled={!canSubmit}
                className="flex-1 min-h-[44px] bg-brand-premium text-brand-bg hover:bg-brand-premium/90 font-semibold"
              >
