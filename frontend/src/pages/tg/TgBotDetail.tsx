  ...
  const load = useCallback(async () => {
    ...
    try {
      const [{ data: b }, { data: pos }] = await Promise.all([
        ...
        api.get<PositionResponse[]>(`/trading/bots/${id}/positions?status=open`),
      ]);
      setBot(b);
      setPositions(pos);
    } finally {
  ...
  return (
    <>
      ...
      <div className="space-y-3 p-4">
        <TgPnlWidget totalPnl={bot.total_pnl} winRate={bot.win_rate} totalTrades={bot.total_trades} />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm capitalize text-gray-300">{bot.status}</span>
          </div>
          <button
            onClick={toggle}
            disabled={toggling}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className={cn(
                        ...
                        pos.side === 'long' ? 'text-[#00E676]' : 'text-[#FF1744]',
                      )}>
                    ...
                    <div className="text-right">
                      <p className={cn(
                        ...
                        pos.unrealized_pnl >= 0 ? 'text-[#00E676]' : 'text-[#FF1744]',
                      )}>
