...
export default function TgDashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>({ totalPnl: 0, winRate: 0, totalTrades: 0, activeBots: 0 });
  ...
  const load = useCallback(async () => {
    try {
      ...
      const winRates = bots.filter((b) => b.total_trades > 0).map((b) => b.win_rate);
      const winRate = winRates.length ? winRates.reduce((s, v) => s + v, 0) / winRates.length : 0;
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <TgCard>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-gray-400">Total trades</span>
                </div>
                <p className="mt-1 font-['JetBrains_Mono'] text-2xl font-bold text-white">
                  {stats.totalTrades}
                </p>
                    ...
                    <TgCard key={bot.id} onClick={() => navigate(`/tg/bots/${bot.id}`)}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-white">{bot.mode.toUpperCase()}</span>
                        </div>
