"""Grid search optimizer for SMCSweepScalperStrategy.

Runs backtests across parameter combinations using multiprocessing.
Reads candles from data/candles/*.parquet cache.

Scalper strategy targets 5-10 signals/day/symbol on TF=5m.
On 180 days of 5m candles that's ~900-1800 trades total.

Usage:
    python backend/scripts/optimize_smc_sweep_scalper.py \
...
# === Grid definitions ===
# Phase 1 — coarse: entry frequency / risk / confluence threshold
PHASE1_COARSE_GRID: dict[str, list[Any]] = {
    "sweep.lookback": [10, 20, 30],
    "entry.min_confluence": [1.0, 1.5, 2.0, 2.5],
    "risk.sl_max_pct": [0.008, 0.012, 0.015, 0.02],
    "risk.tp1_r_mult": [0.8, 1.0, 1.5],
    "risk.tp2_r_mult": [1.5, 2.0, 3.0],
    "entry.cooldown_bars": [2, 3, 5],
}

# Phase 2 — fine: filters / confirmation window / SL buffer
PHASE2_FINE_ADDITIONAL_GRID: dict[str, list[Any]] = {
    ...
    "filters.trend_filter_enabled": [True, False],
    "risk.trailing_atr_mult": [1.0, 1.5, 2.0],
    "risk.sl_atr_buffer": [0.2, 0.3, 0.5],
}

# Phase 3 — fine tuning partial close percentages & FVG min size
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Grid search optimizer for SMCSweepScalperStrategy",
    )
    parser.add_argument(
        "--symbols",
        required=True,
...
def load_base_config() -> dict:
    """Load default config for smc_sweep_scalper from seed_strategy.py."""
    from scripts.seed_strategy import STRATEGIES

    for s in STRATEGIES:
        if s["slug"] == "smc-sweep-scalper":
            return copy.deepcopy(s["default_config"])
    raise RuntimeError("smc-sweep-scalper not found in seed STRATEGIES")
...
def score_scalping(metrics: dict) -> float:
    """Score configs for scalping. Scalpers need high trade frequency.

    Note: win_rate is 0-1 fraction.
    """
    ...
    # Need enough signals for a scalper
    if trades < 50:
        return -999.0
    ...
    # Target frequency bonus: 900-2000 trades over 180 days = 5-11/day
    if 900 <= trades <= 2500:
        score += 15
    elif 500 <= trades < 900:
        score += 5
...
def run_one_backtest(args: tuple) -> dict:
    ...
    try:
        ...
        engine = get_engine("smc_sweep_scalper", config)
...
def _save_progress(symbol: str, timeframe: str, phase: str, results: list[dict]) -> Path:
    ...
    path = RESULTS_DIR / f"smc_scalper_{symbol}_{timeframe}_{phase}_{ts}.json"
...
def _save_final(
    ...
    json_path = RESULTS_DIR / f"smc_scalper_{symbol}_{timeframe}_{ts}.json"
    payload = {
        "symbol": symbol,
        "timeframe": timeframe,
        "days_back": days_back,
    ...
    md_path = RESULTS_DIR / f"smc_scalper_{symbol}_{timeframe}_{ts}.md"
    lines = [
        f"# SMC Sweep Scalper -- {symbol} {timeframe}m -- {ts[:8]}",
        "",
        f"**Runtime:** {runtime_seconds:.1f}s  |  **Final phase:** {final_phase}  |  **Top-{top_n}**",
        "",
        "## Top Configs",
        "",
        "| # | Score | PnL% | DD% | WR | PF | Sharpe | Trades | AvgDur | sweep_lb | min_conf | sl_max | tp1_R | tp2_R | cooldown |",
        "|---|-------|------|-----|-----|-----|--------|--------|--------|----------|----------|--------|-------|-------|----------|",
    ]
